...
import { findBaseInteriorById } from '../utils/manifest'
...
export default defineEventHandler(async (event) => {
  return withErrorHandling(async () => {
    ...
    if (!parsed.success) {
      throw new ApiError('EMPTY_PROMPT', 'Body must include { baseId, prompt }')
    }
    const { baseId, prompt } = parsed.data

    const publicRoot = resolve(process.cwd(), 'public')
    const base = await findBaseInteriorById(publicRoot, baseId)
    const { buffer, mimeType } = await loadSourceImage(
      `/base-interiors/${base.filename}`,
      publicRoot,
    )
